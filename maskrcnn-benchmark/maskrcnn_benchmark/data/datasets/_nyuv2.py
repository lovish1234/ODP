...
author: Mihai Suteu
date: 15/05/19
Source: https://github.com/xapharius/pytorch-nyuv2/blob/master/nyuv2/nyuv2.py
"""
...
class NYUv2(Dataset):
    ...
    def __getitem__(self, index: int):
        def folder(name): return os.path.join(self.root, f"{self._split}_{name}")
        imgs = dict()

        if self.rgb:
            imgs["rgb"] = Image.open(os.path.join(
                folder("rgb"), self._files[index]))
        ...
        if self.surface_normal:
            imgs["sn"] = Image.open(os.path.join(
                folder("sn"), self._files[index]))
    ...
    if not os.path.exists(train_dst) or not os.path.exists(test_dst):
        ...
        if os.path.exists(tar):
            ...
            if not os.path.exists(test_dst):
                _replace_folder(os.path.join(
                    root, "nyu_normals_gt", "test"), test_dst)
...
def _rename_files(folder: str, rename_func: callable):
    ...
    for img_old, img_new in zip(imgs_old, imgs_new):
        shutil.move(os.path.join(folder, img_old),
                    os.path.join(folder, img_new))
...
if __name__ == "__main__":
    # instructions for creating this dataset
    # 1. Create datasets/NYUv2 folder in project root. Simlink alsk works.
    # 2. (optional) If you downloaded the files, put it in datasets/NYUv2 folder. If you didn't download the files, just skip this.
    # 3. In project root, run `python maskrcnn_benchmark/data/datasets/nyuv2.py`, the processing will go on.
    # download and test the dataset
    dataset = NYUv2(root="datasets/NYUv2", download=True)
    print(dataset)